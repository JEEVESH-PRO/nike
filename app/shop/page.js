  ...
  return (
    <div ref={pageRef} className="relative min-h-screen">
      <section className="relative h-[50vh] md:h-[60vh] flex items-end overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom,rgba(230,0,0,0.08),transparent_50%)]" />

        <div ref={heroRef} className="relative z-10 px-6 md:px-12 pb-16 md:pb-20 max-w-7xl mx-auto w-full">
          <motion.span {...fadeUp(0)} className="label-caps text-accent">Catalog</motion.span>
          <motion.h1 {...fadeUp(0.1)} className="headline-lg text-white mt-3">All Sneakers</motion.h1>
          <motion.p {...fadeUp(0.2)} className="text-sm text-white/40 font-body mt-4">{filtered.length} styles · Engineered for movement</motion.p>
        </div>
      </section>

      <div className="px-6 md:px-12 py-16 md:py-20">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="mb-12 space-y-4">
          ...
          {filtered.length === 0 ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-32">
              ...
              <p className="text-2xl font-display text-white/30 mb-2">No sneakers found</p>
              <p className="text-xs text-white/20 font-body">Try a different filter or search term</p>
            </motion.div>
          ) : (

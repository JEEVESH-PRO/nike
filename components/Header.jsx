      ...
      >
        <div className="flex items-center justify-between px-5 md:px-8 py-3.5 md:py-4">
          ...
          <nav ref={navRef} onMouseMove={onNavMove} onMouseLeave={onNavLeave} className="hidden lg:flex items-center gap-8 relative">
            {sections.map((s) => (
              <button
                ...
                className={`nav-link relative text-[10px] uppercase tracking-[0.2em] font-body transition-colors duration-300 ${
                  activeSection === s.id ? 'text-white' : 'text-white/40 hover:text-white/80'
                }`}
              >
                {s.label}
              </button>
            ...
            <div
              ref={indicatorRef}
              className="absolute -bottom-1 h-[2px] bg-accent rounded-full transition-all duration-500 ease-out opacity-0"
              style={{ width: 0, transform: 'translateX(0)' }}
            />
          </nav>

          <div className="flex items-center gap-3 z-10">
            <TransitionLink href="/shop" className="hidden md:flex label-caps text-white/40 hover:text-white transition-colors" data-cursor="shop">
              Shop
            </TransitionLink>

            <TransitionLink href="/wishlist" className="relative p-2 text-white/40 hover:text-white transition-colors" data-cursor="view">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4">
                <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" />
              </svg>
              {wishlisted.length > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-accent text-white text-[8px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">{wishlisted.length}</span>
              )}
            </TransitionLink>

            <TransitionLink href="/cart" className="relative p-2 text-white/40 hover:text-white transition-colors" data-cursor="view">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4">
                <path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z" /><line x1="3" y1="6" x2="21" y2="6" /><path d="M16 10a4 4 0 01-8 0" />
              </svg>
              {totalItems > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-accent text-white text-[8px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">{totalItems}</span>
              )}
            </TransitionLink>
        ...
        <div className="absolute bottom-0 left-4 right-4 h-px bg-white/5 overflow-hidden rounded-full">
          <div className="h-full bg-accent transition-all duration-150 rounded-full" style={{ width: `${progress * 100}%` }} />
        </div>
      </motion.header>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(230,0,0,0.1),transparent_50%)]" />
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center px-8 md:px-16">
              ...
              >
                <TransitionLink href="/shop" onClick={() => setMenuOpen(false)} className="btn-campaign btn-campaign-primary" data-cursor="shop">Shop Now</TransitionLink>
